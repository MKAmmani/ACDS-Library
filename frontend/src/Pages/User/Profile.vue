...
const initials = computed(() => {
  const n = form.value.name || auth.user?.name || 'MB'
  return n.split(' ').map((w: string) => w[0]).join('').toUpperCase().slice(0, 2)
})

const memberTypeLabel = computed(() => {
  const t = memberInfo.value?.membership_type
  if (!t) return 'Standard Member'
  return t.charAt(0).toUpperCase() + t.slice(1) + ' Member'
})
      ...
      <div class="pt-10 pb-4 px-6 flex items-end justify-between">
        <div>
          <div class="text-[18px] font-bold text-[var(--navy)]" style="font-family:var(--display)">
            {{ form.name || 'Library Member' }}
          </div>
          <div class="text-[12.5px] text-[var(--muted)] mt-0.5 flex items-center gap-2">
            <span class="badge b-green">{{ memberTypeLabel }}</span>
            <span>· {{ memberInfo?.member_number ?? auth.user?.member_number ?? 'N/A' }}</span>
          </div>
        </div>
        <div class="text-[12px] text-[var(--green)] flex items-center gap-1.5">
          <span class="w-1.5 h-1.5 rounded-full bg-[var(--green)]"></span> Good Standing
        </div>
      </div>
    ...
    <div v-if="activeTab==='profile'" class="bg-white border border-[var(--line)] rounded-xl overflow-hidden" style="box-shadow:var(--sh1)">
      ...
      <div v-else class="p-6">
        ...
        <div class="bg-[var(--bg)] border border-[var(--line)] rounded-[10px] p-4 mb-6">
          <div class="text-[11px] font-bold tracking-[.07em] uppercase text-[var(--muted)] mb-3">Membership Details</div>
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div v-for="[k,v] in [
              ['Member No.', memberInfo?.member_number ?? auth.user?.member_number ?? 'N/A'],
              ['Membership Type', memberTypeLabel],
              ['Member Since', formatDate(memberInfo?.created_at ?? null)],
              ['Expires', formatDate(memberInfo?.membership_expires_at ?? null)],
              ['Borrow Limit', '5 books'],
              ['Renewal Limit', '2 per item'],
            ]" :key="k">
        ...
        <div class="flex gap-3">
          ...
          <button class="btn btn-ghost" @click="form.name=memberInfo?.name??''; form.phone=memberInfo?.phone??''; form.address=memberInfo?.address??''; saveError=''">Discard</button>
        </div>
      </div>
    </div>
      ...
      <div class="p-6">
        <div class="flex flex-col gap-0">
          <div v-for="[key,label,desc] in [
            ['emailDue',       'Email — Due date reminders',    'Receive an email 3 days before a book is due.'],
            ['emailReady',     'Email — Reservation ready',     'Notify me when a reserved book is ready for collection.'],
            ['smsOverdue',     'SMS — Overdue alerts',          'Receive an SMS when a book becomes overdue.'],
            ['emailNewsletter','Email — Library newsletter',    'Monthly updates on new acquisitions and events.'],
