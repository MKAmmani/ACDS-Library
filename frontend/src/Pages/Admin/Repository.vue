...
function emptyBulkForm() {
  return { subject_area: '', language: 'English', format: 'PDF Document' }
}
...
async function uploadBulkFiles() {
  ...
  for (const entry of bulkFiles.value) {
    ...
    try {
      ...
      await apiUpload<RepoItem>('/admin/repository', fd, auth.token!)
      entry.status = 'done'
    } catch (e: any) {
      entry.status = 'error'
...
const readerDoc       = ref<RepoItem | null>(null)
...
async function openDoc(doc: RepoItem) {
  ...
  try {
    ...
    const res = await fetch(`${BASE_URL}/repository/${doc.id}/inline`, {
...
<template>
  ...
  <div :class="['drawer', { open: bulkDrawerOpen }]">
    <div class="dh">
      <div>
        <div class="dh-t">Bulk Upload Documents</div>
    ...
    <div class="db">
      ...
      <div class="db-sec">
        ...
        <div class="form-grid">
          ...
          <div class="fg">
            <label class="fl">Document Format</label>
            <select class="fs" v-model="bulkForm.format">
              <option>PDF Document</option><option>eBook</option><option>Journal</option>
              <option>Thesis / Dissertation</option><option>Research Report</option>
              <option>Working Paper</option><option>Conference Paper</option>
              <option>Government Document</option>
