...
function emptyForm() {
  return {
    ...
    is_archived: false, archive_reason: '',
  }
}
...
function openEdit(book: Book) {
  ...
  Object.assign(form, {
    ...
    is_archived:      book.is_archived ?? false,
...
async function fetchBooks() {
  ...
  try {
    ...
    if (filterArchived.value) params.set('archived',     filterArchived.value)
...
function coverLabel(title: string): string {
  ...
  const [first] = words
  return first ? first.substring(0, 4) + '.' : title.substring(0, 4)
}
...
watch([searchQuery, filterFormat, filterSubject, filterArchived], () => {
  currentPage.value = 1
...
<template>
  ...
  <div class="toolbar">
    ...
    <select class="sel" v-model="filterArchived">
    ...
    <table class="tbl">
      ...
      <tbody>
        ...
        <tr v-else v-for="book in books" :key="book.id">
          ...
          <td>
            <span :class="['badge', bookStatus(book).cls]">{{ bookStatus(book).label }}</span>
            <span v-if="book.is_archived" class="badge b-gray" style="margin-left:4px" :title="book.archive_reason ?? ''">Archived</span>
          </td>
          <td>
    ...
    <div class="db">
      ...
      <div class="db-sec">
        ...
        <div class="form-grid">
          ...
          <div class="fg">
            <label class="fl">Subject Area</label>
      ...
      <!-- Archive Status -->
      <div class="db-sec">
        <div class="db-sec-h"><LucideIcon name="archive" /> Archive Status</div>
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;color:var(--ink);cursor:pointer;margin-bottom:10px">
          <input type="checkbox" v-model="form.is_archived" />
          Move this title out of open-shelf circulation into archival storage
        </label>
        <div class="form-grid" v-if="form.is_archived">
          <div class="fg col2">
            <label class="fl">Reason for Archiving</label>
            <select class="fs" v-model="form.archive_reason">
              <option value="">Select a reason…</option>
              <option v-for="r in ARCHIVE_REASONS" :key="r">{{ r }}</option>
            </select>
          </div>
        </div>
      </div>
