...
// ── Stats ─────────────────────────────────────────────────────────────────────
interface EResourceStats { total: number; storage_bytes: number; downloads_30d: number }
...
async function fetchStats() {
  try {
    const s = await apiGet<EResourceStats>('/e-resources/stats')
...
const editingDoc = ref<EResourceItem | null>(null)
...
function emptyBulkForm() {
  return { subject_area: '', language: 'English', format: 'E-Resource' }
}
...
function handleBulkFilesInput(e: Event) {
  ...
  for (const file of files) {
    bulkFiles.value.push({ file, title: titleFromFilename(file.name), status: 'pending' })
  }
...
async function uploadBulkFiles() {
  if (!bulkForm.subject_area.trim() || !bulkFiles.value.length) return
  ...
  for (const entry of bulkFiles.value) {
    ...
    try {
      ...
      Object.entries(bulkForm).forEach(([k, v]) => { if (v) fd.append(k, String(v)) })
      fd.append('file', entry.file)
      await apiUpload<EResourceItem>('/admin/e-resources', fd, auth.token!)
      entry.status = 'done'
    } catch (e: any) {
      entry.status = 'error'
...
const deletingDoc     = ref<EResourceItem | null>(null)
...
const readerDoc       = ref<EResourceItem | null>(null)
...
function emptyForm() {
  return {
    ...
    language: 'English', format: 'E-Resource',
  }
}
...
function openEdit(doc: EResourceItem) {
...
async function fetchDocs() {
  ...
  try {
    ...
    const res     = await apiGet<EResourcePage>(`/e-resources?${params}`)
    docs.value        = res.data
...
async function save() {
  if (!form.title.trim() || !form.authors.trim() || !form.subject_area.trim()) {
    saveError.value = 'Title, Author(s) and Subject Area are required.'
    return
  }
  saving.value    = true
  saveError.value = ''
  try {
    ...
    const path = editingDoc.value
      ? `/admin/e-resources/${editingDoc.value.id}`
      : '/admin/e-resources'

    await apiUpload<EResourceItem>(path, fd, auth.token!)
...
function promptDelete(doc: EResourceItem) {
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await apiDelete(`/admin/e-resources/${deletingDoc.value.id}`, auth.token!)
...
async function openDoc(doc: EResourceItem) {
  ...
  try {
    ...
    const res = await fetch(`${BASE_URL}/e-resources/${doc.id}/inline`, {
...
async function downloadDoc(doc: EResourceItem) {
  try {
    const res  = await fetch(`${BASE_URL}/e-resources/${doc.id}/download`, {
...
// ── Helpers ───────────────────────────────────────────────────────────────────
function docKind(doc: EResourceItem): string {
...
function docIcon(doc: EResourceItem): string {
...
<template>
  ...
  <div class="shead">
    <div>
      <h2>E-Resources</h2>
      <p>Upload and store external e-books, PDFs &amp; documents sourced from outside the institution</p>
    </div>
  ...
  <div class="kpis" style="grid-template-columns:repeat(3,1fr)">
    <div class="kpi">
      <div class="kpi-top">
        ...
        <div class="kpi-ic t-blue"><LucideIcon name="link" /></div>
      </div>
      <div class="kpi-l">Resources Stored</div>
    </div>
    <div class="kpi">
      <div class="kpi-top">
  ...
  <div class="toolbar" style="margin-bottom:16px">
    <div class="search-in">
      ...
      <input v-model="searchQuery" placeholder="Search external resources…" />
    </div>
    <div class="fbtns">
      <div
  ...
  <div class="repo-grid">
    ...
    <div v-else-if="!docs.length" class="repo-empty">
      <LucideIcon name="link" style="width:32px;height:32px;margin-bottom:8px;display:block;margin-inline:auto" />
      No e-resources yet — click <b>Upload Files</b> to add the first one.
    </div>
  ...
  <div :class="['drawer', { open: drawerOpen }]">
    <div class="dh">
      <div>
        <div class="dh-t">{{ editingDoc ? 'Edit Resource' : 'Upload E-Resource' }}</div>
        <div class="dh-s">{{ editingDoc ? 'Update resource record' : 'Add a new external resource' }}</div>
      </div>
      ...
      <div class="db-sec">
        ...
        <div class="form-grid">
          ...
          <div class="fg">
            ...
            <input class="fi" v-model="form.publisher" placeholder="e.g. Open Access Press" />
          </div>
          <div class="fg">
    ...
    <div class="df">
      ...
      >
        ...
        {{ saving ? 'Saving…' : editingDoc ? 'Save Changes' : 'Upload Resource' }}
      </button>
  ...
  <div :class="['drawer', { open: bulkDrawerOpen }]">
    <div class="dh">
      <div>
        <div class="dh-t">Bulk Upload E-Resources</div>
        <div class="dh-s">Upload any file type at once — shared details apply to every file</div>
      </div>
    ...
    <div class="db">
      <!-- Shared metadata -->
      <div class="db-sec">
        <div class="db-sec-h"><LucideIcon name="file-text" /> Shared Details (applied to every file)</div>
        <div style="font-size:12px;color:var(--faint);margin:-6px 0 12px">
          Author, publisher and year are left blank here — fill those in per document later via Edit.
        </div>
        <div class="form-grid">
          <div class="fg col2">
            <label class="fl">Subject Area <span class="req">*</span></label>
            <input class="fi" v-model="bulkForm.subject_area" placeholder="e.g. Democracy &amp; Governance" />
          </div>
          <div class="fg col2">
            <label class="fl">Language</label>
            <select class="fs" v-model="bulkForm.language">
              <option>English</option><option>Hausa</option><option>Arabic</option>
              <option>French</option><option>Yoruba</option><option>Igbo</option>
            </select>
          </div>
      ...
      <!-- Files -->
      <div class="db-sec">
        <div class="db-sec-h"><LucideIcon name="upload" /> Files <span style="font-weight:400;text-transform:none;letter-spacing:0;color:var(--faint)"> — any document type, or a whole folder · max 1 GB each</span></div>
        ...
        <div style="display:flex;gap:10px;margin-bottom:12px">
          <label class="up-btn">
            ...
            Choose files…
            <input
              ref="bulkFileInput" type="file" multiple
              style="display:none"
              @change="handleBulkFilesInput"
            />
          </label>
          <label class="up-btn">
            ...
            Choose folder…
            <input
              ref="bulkFolderInput" type="file" webkitdirectory directory multiple
              style="display:none"
              @change="handleBulkFilesInput"
            />
          </label>
        ...
        <div v-else style="display:flex;flex-direction:column;gap:8px">
          <div
            v-for="(entry, idx) in bulkFiles" :key="idx"
            style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:var(--bg);border-radius:var(--r2)"
          >
            <LucideIcon name="file-text" class="ic-sm" style="flex-shrink:0;color:var(--muted)" />
            <input
              class="fi" v-model="entry.title" :disabled="entry.status !== 'pending'"
              style="flex:1;min-width:0" placeholder="Document title"
            />
            <span style="font-size:11px;color:var(--faint);flex-shrink:0">{{ fmtSize(entry.file.size) }}</span>
    ...
    <div class="df">
      <button
        class="btn btn-primary" style="flex:1;justify-content:center"
        :disabled="bulkUploading || !bulkFiles.length || !bulkForm.subject_area.trim()"
  ...
  <div :class="['mscrim', { open: showSavedModal }]" @click.self="showSavedModal = false">
    <div class="modal">
      <div class="modal-b">
        ...
        <div class="modal-t">{{ editingDoc ? 'Resource Updated' : 'Resource Uploaded' }}</div>
        <div class="modal-s">
          {{ editingDoc
              ? 'The e-resource record has been updated successfully.'
              : 'The document has been added to the e-resources collection.' }}
        </div>
        <div class="modal-f">
  ...
  <div :class="['mscrim', { open: showDeleteModal }]" @click.self="showDeleteModal = false">
    <div class="modal">
      <div class="modal-b">
        ...
        <div class="modal-t">Delete Resource?</div>
        <div class="modal-s">
